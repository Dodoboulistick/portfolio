<template>
  <section class="container py-10 px-3" id="portfolio" aria-label="Portfolio">
    ...
    <div class="grid xl:grid-cols-2 gap-x-20 items-center py-5 md:py-10">
      <div class="col-span-1">
        <img
          ...
          loading="lazy"
        />
        <p class="text-sm md:text-base text-center opacity-50 italic my-5">
          {{ t("portfolio.preview") }}
        </p>
      </div>
      <div class="col-span-1 grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-8">
        <div
          v-for="item in items"
        ...
        >
          ...
          >
            <h3 class="lg:text-2xl font-semibold truncate">
              {{ t(item.title) }}
            </h3>
            <a
              v-if="item.index === 0"
              ...
              target="_blank"
              rel="noopener"
              class="flex flex-nowrap items-center gap-1 lg:text-lg text-blue-600 hover:bg-blue-600/10 px-2 rounded-md transition-all duration-300"
              :aria-label="t('portfolio.cta') + ' ' + t(item.title)"
              >{{ t("portfolio.cta") }}
              <i class="bi bi-arrow-up-right text-sm" aria-hidden="true"></i
            ></a>
          </div>
          <p class="opacity-70 text-sm md:text-base">
            {{ t(item.description) }}
          </p>
        </div>
      </div>
